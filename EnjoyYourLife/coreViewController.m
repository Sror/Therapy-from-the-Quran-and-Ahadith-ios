...
//    
    [self.pageSlider setFrame:CGRectMake(30, 10, 260, 25)];
    [self.pageSlider setMaximumTrackTintColor:[UIColor colorWithRed:111.0/255.0 green:151.0/255.0 blue:164.0/255.0 alpha:0.5]];
    [self.pageSlider setMinimumTrackTintColor:[UIColor colorWithRed:111.0/255.0 green:151.0/255.0 blue:164.0/255.0 alpha:1]];
    //    [self.pageSlider setMinimumTrackTintColor:[UIColor clearColor]];
    [self.pageSlider setThumbImage:[UIImage imageNamed:@"slider-btn@2x.png"] forState:UIControlStateNormal];
    ...
    chapterPrefix = 1;
    totalChapters = 96 + chapterPrefix;
...
//    }
//
    self.allChaptersNo = [[NSMutableArray alloc]init];
    ...
    if (self.totalPages == 0) {
        ...
        //store all page nos in array
        [self getAllChapterNo];
    }
    pageNumber=1;
    self.fontSliderBar.hidden = YES;
    [super viewDidLoad];
...
- (void) chapterSelectdAction:(NSNotification*)notif{
    ...
    self.pageNo.text=[NSString stringWithFormat:@"%i of %i",pageNumber,self.totalPages];
    self.chapterNo.text=[NSString stringWithFormat:@"%i.%@",self.ChID, self.chTitle];
    //
...
-(void) getChapter: (int)cid
{
    @try {
        ...
        NSString *text=[self.dataSource createPage:cid childId:-1 bit:0];
        NSAttributedString * drawingText;
        [self.userDefaults setInteger:cid - 1 forKey:@"CurrentChapter"];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController *)viewController
{
    ...
    else if(self.top==0 && self.ChID>1)
    {
        ...
        dataViewController = [[DataViewController alloc] initWithAttributedString:drawingAttrText];
        self.chapterNo.text=[NSString stringWithFormat:@"%i.%@",self.ChID, self.chTitle];
        [self.userDefaults setInteger:self.ChID - 1 forKey:@"CurrentChapter"];
...
- (NSAttributedString *) getAttributedSubStringToFitVisibleRectFromString:(NSAttributedString*)text InView:(UIView*)textContainingView ForPageNumber:(int)page{
    ...
    int tvH = (int)roundf(self.textView.frame.size.height);
    if(tvH>950)
        tvH = 60;
    else tvH = 40;
    CGRect textFrame = CGRectInset(bounds, 20, tvH);
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathAddRect(path, NULL, textFrame );
...
- (void)sliderDidEndSliding{
    ...
    [self getChapter:self.ChID];
    self.top = value - [[self.allChaptersNo objectAtIndex:self.ChID-1 ]intValue ] ;
    if(self.top>= self.pages.count)
        self.top = self.pages.count-1;
    if(self.top<0)
        self.top=0;
...
-(void) getAllChapterNo
{
    for (int i=0; i<totalChapters; i++) {
        [self.allChaptersNo addObject:[NSNumber numberWithInt:[self.dataSource getChapterPageNo:i+1]]];
    }
}
-(int) getChapterAtPage:(int)currentpgno
{
    int c=1,chapterStartNo;
    for (c=1; c<totalChapters; c++) {
        
        chapterStartNo = [[self.allChaptersNo objectAtIndex:c ]intValue ];
        if(chapterStartNo>=currentpgno)
            break;
    }
    return c;
}
