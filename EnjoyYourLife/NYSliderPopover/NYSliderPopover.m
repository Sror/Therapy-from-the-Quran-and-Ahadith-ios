...
- (NYPopover *)popover
{
    if (_popover == nil) {
        ...
        _popover = [[NYPopover alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y - 32, 70, 40)];
...
{
    [self performSelector:@selector(hidePopoverAnimated:) withObject:[NSNumber numberWithBool:YES] afterDelay:1.0];
    if([self.delegate respondsToSelector:@selector(sliderDidEndSliding)])
        [self.delegate sliderDidEndSliding];
